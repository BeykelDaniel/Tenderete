<template>
    <div class="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-gray-100 flex flex-col h-[700px]">
        <!-- Forum Table (Messages) -->
        <div ref="forumBox" class="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 bg-[#f8fafc]/50 custom-scrollbar">
            <div v-for="msg in mensajesList" :key="msg.id" 
                ...
                <div :class="['flex flex-col', msg.user_id == currentUserId ? 'items-end' : '', 'max-w-[75%]']">
                    <div class="flex items-center gap-2 mb-1">
                        ...
                        <span class="text-[10px] font-bold text-gray-300">{{ formatTime(msg.created_at) }}</span>
                    </div>
        ...
        <div class="p-6 bg-white border-t border-gray-100">
            <form @submit.prevent="enviar" class="relative flex items-center gap-3">
                <input v-model="nuevoMensaje" type="text" placeholder="Escribe algo en el foro..."
                ...
                <button type="submit" :disabled="enviando || !nuevoMensaje.trim()"
                        class="bg-indigo-600 text-white w-16 h-16 rounded-full flex items-center justify-center shadow-lg hover:bg-indigo-700 hover:scale-110 transition-all shrink-0">
                    <i v-if="!enviando" class="bi bi-chat-dots-fill text-2xl"></i>
...
<script>
export default {
    props: ['initialMensajes', 'currentUserId', 'routeStore', 'routeNuevos', 'csrf'],
    data() {
        return {
    ...
    methods: {
        ...
        poll() {
            fetch(`${this.routeNuevos}?last_id=${this.lastId}`)
                .then(r => r.json())
                .then(data => {
                    const posts = data.mensajes || data;
                    if (posts.length > 0) {
